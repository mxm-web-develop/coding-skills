...
下面五个案例展示 AI Flow 在真实项目里怎么跑。每个案例都按"用户说什么 → AI Flow 怎么做"来写，覆盖最常见的几种情况。
...
- **AI Flow 跑哪些模块**：模块 1 → 模块 3 → 模块 4 → 模块 5 → 模块 6 → 模块 7 → 模块 8
- **你会看到的对话**：
  - 先问用户、问题、范围、非目标和验收，不会一上来就问"用 Vue 还是 React"。
  - 技术选型时告诉你"为什么推荐 A"，并列出 B/C 的取舍，等你确认。
  - 你确认 V1 目标后，拆成 5–8 个 Work Item，每个都能独立验收。
  - 每完成一项，跑真实测试，把结果给你看；不会直接说"做完了"。
...
- **AI Flow 跑哪些模块**：模块 1 → 模块 2 → 模块 4 → 模块 3
- **你会看到的对话**：
  - Agent 只读扫描，输出一份"项目基线"：当前版本、已实现功能、测试情况、文档冲突、待确认事项。
  - 区分代码直接证明的、合理推断的、需要你确认的，三类不会混在一起。
  - 如果有散落历史文档，问你是"保持原样 / 只读盘点 / 批准后归档"——选哪个都行，默认不动文件。
  - 你对基线满意后，才开始讨论新版本的需求和计划。
...
- **AI Flow 跑哪些模块**：模块 1 → 模块 4（轻量）→ 模块 5 → 模块 6 → 模块 7 → 模块 8
- **你会看到的对话**：
  - Agent 先写一个能稳定复现的失败用例（测试先行）。
  - 定位根因，给出修复方案并修改代码。
  - 跑全部相关测试，证据（命令、退出码、日志、Git SHA）写入 `.ai-flow/evidence/`。
  - 独立评审通过后，做原子提交并关联 Work Item。
  - 看板把这个 Bug 的状态更新为"已修复 + 测试通过"。
...
- **AI Flow 跑哪些模块**：模块 1（识别"恢复"意图）→ 读取 Checkpoint → 模块 6 继续
- **你会看到的对话**：
  - 先告诉你当前大版本、已完成任务、正在做的任务、阻塞项、上次保存的 Git SHA。
  - 核对当前代码版本是否与暂停时一致，不一致会先解释差异而不是强行恢复。
  - "继续"不会被当成"批准所有待确认事项"——技术方案、删除、移动等仍需要单独确认。
  - 切换 IDE 后看到的是同一份 `.ai-flow/` 状态和同一份看板，没有第二份进度文档。
...
- **AI Flow 跑哪些模块**：模块 1 → 模块 8（含门禁核对）→ 模块 7（Git 准备）
- **你会看到的对话**：
  - Agent 用真实命令汇总测试结果：哪个版本、哪条命令、退出码、日志哈希，**不是它自己说"通过了"**。
  - 核对发布门禁：Schema 合法、追踪链完整、所有 Work Item 有可信证据、看板新鲜、阻塞项已解决。
  - 任何一项不通过都会停下来告诉你还差什么；全部通过后会列出"接下来要做的事"（创建 tag、写 changelog 等），每一步等你点头。
  - **默认不会自动 push、merge、tag、发布或部署**——这些动作都明确分别需要授权。
