...
| 用强模型制定计划，再换模型执行 | 保存可执行计划、必要项目知识和失败时的处理边界；换编辑器后仍接续同一任务 |
...
AI Flow 的核心用途是让项目记忆和开发安排留在项目里。你可以用擅长规划的模型讨论需求、确定设计，再切换到较便宜的模型实现；编辑器也可以更换。

v1.1.0 会为待执行任务保存接口约束、代码入口、实现步骤、边界情况、测试命令和停止条件。执行者读取当前任务的完整资料；遇到设计冲突或多次失败，就带着证据交回规划者。执行者与评审者分开记录，用户验证通过后再归档。

自动扫描得到的是候选信息，核实后才能作为项目事实使用；来源变化会使相关交接计划失效。测试样例不会再自动变成项目技术栈。

这些机制支持跨模型协作，但不能保证任意模型都能完成任意任务，也不会自动调用或切换模型。目前没有 MiniMax/K2 的实测能力与成本对比，不能据此承诺节省比例。先用一个代表性小任务检查执行能力，再逐步扩大范围。见[跨模型使用与验证](docs/cross-model-development.md)。
...
```bash
curl -fsSL https://raw.githubusercontent.com/mxm-web-develop/coding-skills/main/install/get.sh | AI_FLOW_COMMAND=update AI_FLOW_VERSION=v1.1.0 sh
```
