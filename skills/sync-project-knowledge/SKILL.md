...
7. Read [references/board-contract.md](references/board-contract.md), then run `.ai-flow/bin/flowctl render-board --root <root>`. Never write or patch a board directly. `render-board` writes the four management boards in `docs/board/`, plus a version plan index at `docs/board/PLANS.md` and one natural-language plan document per un-released version at `docs/board/plans/v<version>.md`.
...
9. Verify that the files in `docs/board/` use natural-language summaries and compact tables for versions, tasks, decisions, tests, releases, and the version plan index while remaining fully traceable to machine objects. Each un-released version has a companion plan document under `docs/board/plans/` written for human readers (PM, dev team, stakeholders) — no raw IDs, no internal object names in the main text. Summarize legacy history by version without expanding archived prose back into active context.
