...
日常使用先看 [v1.0.0 使用方式](../README.md#v100-使用方式)。汇报计划只读，独立调研停在建议，新功能在测试与评审后等待人工验证，用户通过后自动收尾；升级旧项目先[核对资料再续接](upgrade.md)。下面是维护与排错时按需阅读的完整说明。
...
`evidence verify` 重新计算日志 SHA-256；要求当前版本时，同时核对 HEAD 与实际代码内容。
...
测试和评审通过后，AI 提供入口、前置条件、操作步骤、每步预期结果和已知限制。用户验证失败则修复同一任务；明确通过后更新当前能力与计划，并归档本任务的过程材料，保留追溯索引。未给出实际反馈时，只能显示待人工验证。提交可以单独授权，发布也可以延后。

详见[人工验收步骤](v1-acceptance.md)。
