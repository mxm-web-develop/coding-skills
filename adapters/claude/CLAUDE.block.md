...
This repository uses AI Flow. Route project status, features, bugs, refactors, tests, reviews, Git integration, versions, releases, and workflow documentation through `/ai-flow`. If `.ai-flow/manifest.yaml` is missing, `/ai-flow` must dispatch initialization first.
